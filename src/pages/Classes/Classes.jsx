...
export default function Classes() {
  ...
  return (
    <div className="relative min-h-screen w-full overflow-hidden bg-gradient-to-br from-[#F3F0FF] via-[#FAF9FF] to-[#FFF1F8]">
      {/* ambient background blobs, echoes the boards page */}
      ...
      <div style={{ padding: "1.5rem" }} className="relative w-full px-6 py-10 sm:px-10 lg:px-16">
        <button
          ...
          className="group mb-6 flex items-center gap-2 rounded-xl border border-slate-200 bg-white/70 px-3.5 py-2 text-sm font-semibold text-slate-600 backdrop-blur-sm transition hover:border-violet-200 hover:bg-white hover:text-violet-600 cursor-pointer"
        >
          <ArrowLeft className="h-4 w-4 transition-transform duration-200 group-hover:-translate-x-0.5" />
          Back
        </button>

        <div style={{ marginTop: "1rem" }} className="mb-8 mt-2 flex items-center gap-2 text-sm text-slate-500">
        ...
        {!loading && !errorMsg && classes.length === 0 && (
          <div className="flex flex-col items-center justify-center rounded-2xl bg-white px-6 py-20 text-center shadow-sm ring-1 ring-slate-100">
            ...
            <p className="text-sm text-slate-500">
              Classes for this board will show up here.
            </p>
          </div>
        )}

        {!loading && !errorMsg && classes.length > 0 && (
          <div style={{ marginTop: "1.5rem" }} className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {classes.map((item, i) => {
              ...
              return (
                <div
                  ...
                  className="group animate-[fadeInUp_0.4s_ease-out_backwards] cursor-pointer rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-100 transition-all duration-300 hover:-translate-y-1 hover:shadow-xl flex flex-col justify-between h-[150px]"
                >
                  <div
